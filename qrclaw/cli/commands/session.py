...
def handle(args: str, current_session: Session, console: Console) -> Session:
    """
    ...
    if subcommand == "list":
        return _cmd_list(current_session, console)
    elif subcommand == "new":
        return _cmd_new(sub_args, current_session, console)
    elif subcommand == "switch":
        return _cmd_switch(sub_args, current_session, console)
    elif subcommand == "delete":
        return _cmd_delete(sub_args, current_session, console)
    else:
...
def _cmd_list(current_session: Session, console: Console) -> Session:
...
def _cmd_new(session_id: str, current_session: Session, console: Console) -> Session:
    # session_id 可选，不传则自动生成
    new_session = Session(session_id=session_id if session_id else None)
    _reinit_logger(new_session.session_id)  # 先切换日志文件
    console.print(f"[bold cyan]已新建并切换到会话: {new_session.session_id}[/bold cyan]")
    return new_session


def _cmd_switch(session_id: str, current_session: Session, console: Console) -> Session:
    if not session_id:
        console.print("[red]用法: /session switch <会话ID>[/red]")
        return current_session

    _reinit_logger(session_id)          # 先切换日志文件
...
def _cmd_delete(session_id: str, current_session: Session, console: Console) -> Session:
    if not session_id:
...
def _reinit_logger(session_id: str) -> None:
    setup_logger(
