...
def _build_tooling_section(schemas: list[dict]) -> str:
    """按实际可调用的工具定义生成列表，避免重复维护工具说明。"""
    if not schemas:
        return ""
    return "\n".join([
        "## 可用工具", "工具名称区分大小写，按名称精确调用：",
        *(f"- {schema['function']['name']}: {schema['function'].get('description', '')}" for schema in schemas),
    ])
...
def _build_memory_section(memory_dir: Path, wiki_context: str = "", is_sub_agent: bool = False) -> str:
    ...
    if memory_dir is not None and memory_dir.exists():
        ...
        if content and "暂无页面" not in content:
            ...
            sections.extend([
                ...
                "相关页面正文由当前任务提供。" if is_sub_agent else "需要详细内容时，使用 read_wiki_page 读取对应页面。",
                "",
                content,
            ])
...
def build_system_prompt(
    ...
    """
    from qrclaw.skills.registry import SkillRegistry
    schemas = get_schemas_for_sub_agent() if is_sub_agent else get_schemas()
    ...
    sections = [
        ...
        "",
        _build_tooling_section(schemas),
        "",
        ...
        "",
        _build_memory_section(memory_dir, wiki_context, is_sub_agent),
        "",
