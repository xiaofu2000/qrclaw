...
def _multi_backend_fetch_sync(url: str) -> str:
    ...
    if _has_api_key("FIRECRAWL_API_KEY"):
        try:
            from firecrawl import Firecrawl
            kwargs = {"api_key": os.getenv("FIRECRAWL_API_KEY")}
            api_url = os.getenv("FIRECRAWL_API_URL", "").strip()
            if api_url:
                kwargs["api_url"] = api_url
            result = Firecrawl(**kwargs).scrape(url=url, formats=["markdown"])
            content = ""
            if result and hasattr(result, "markdown"):
                content = result.markdown or ""
            elif isinstance(result, dict):
                content = result.get("markdown", "") or ""
            if content:
                logger.info(f"Firecrawl 获取成功: {url}")
                return content
        except Exception as e:
            logger.debug(f"Firecrawl 获取失败: {e}")
    ...
    if _has_api_key("TAVILY_API_KEY"):
        try:
            resp = requests.post(
                "https://api.tavily.com/extract",
                json={"urls": [url], "api_key": os.getenv("TAVILY_API_KEY")},
                timeout=60,
            )
            if resp.status_code == 200:
                results = resp.json().get("results", [])
                content = (results[0].get("raw_content") or results[0].get("content", "")) if results else ""
                if content:
                    logger.info(f"Tavily 获取成功: {url}")
                    return content
        except Exception as e:
            logger.debug(f"Tavily 获取失败: {e}")
    ...
    if _has_api_key("EXA_API_KEY"):
        try:
            from exa_py import Exa
            result = Exa(api_key=os.getenv("EXA_API_KEY")).get_contents([url], text=True)
            content = ""
            if result and hasattr(result, "results") and result.results:
                content = result.results[0].text or ""
            if content:
                logger.info(f"Exa 获取成功: {url}")
                return content
        except Exception as e:
            logger.debug(f"Exa 获取失败: {e}")
    ...
    # Jina（最终降级）
    try:
        content = _fetch_with_jina_sync(url)
        if content:
            logger.info(f"Jina 获取成功: {url}")
            return content
    except Exception as e:
        logger.debug(f"Jina 获取失败: {e}")
...
class WebExtractArgs(BaseModel):
    urls: list = Field(description="要抓取的网页 URL 列表，最多5个，例如 ['https://example.com']")
    use_compression: bool = Field(default=True, description="是否使用 LLM 压缩大页面内容")


class WebCrawlArgs(BaseModel):
    url: str = Field(description="要爬取的站点根 URL，例如 https://docs.example.com")
    instructions: str = Field(default="", description="爬取指令，描述要提取的内容（可选）")
    max_pages: int = Field(default=10, description="最大爬取页面数（默认10，最大20）")
    use_compression: bool = Field(default=True, description="是否使用 LLM 压缩每个页面内容")
...
@register(
    description=(
        "访问指定网页并提取纯净的 Markdown 正文。支持批量 URL（最多5个），并发抓取后逐个返回。\n"
        "特性：SSRF 防护、多后端 fallback（Firecrawl > Tavily > Exa > Jina）、LLM 内容压缩、Base64 图片清理。"
    ),
    args_model=WebExtractArgs,
)
def web_extract(urls: list, use_compression: bool = True) -> str:
    """
    批量抓取网页正文，并发处理多个 URL。
    """
    if not urls:
    ...
    # 限制最多5个
    urls = [u for u in urls if isinstance(u, str)][:5]
    logger.debug(f"批量抓取: {urls}")

    def _fetch_one(url: str) -> dict:
        try:
            if not _is_safe_url_simple(url):
                return {"url": url, "error": "SSRF 防护：URL 指向私有/内网地址，已被阻止", "content": ""}
            ...
            if not content or content.startswith("[无法获取"):
                return {"url": url, "error": content, "content": ""}
            ...
            logger.info(f"抓取成功: {url}, 长度: {len(content)}")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(urls), 5)) as executor:
        futures = {executor.submit(_fetch_one, url): i for i, url in enumerate(urls)}
        for future in concurrent.futures.as_completed(futures):
            results[futures[future]] = future.result()

    # 单个 URL 直接返回内容，多个返回结构化结果
    if len(results) == 1:
        r = results[0]
        if r["error"]:
            return r["error"]
...
@register(
    description=(
        "爬取整个网站或文档站的多个页面，一次调用获取站点全貌。\n"
        "适合场景：读取文档站所有章节、爬取某模块下的全部页面、重构前了解整个代码库文档。\n"
        "后端：优先 Firecrawl，降级 Tavily。需要 FIRECRAWL_API_KEY 或 TAVILY_API_KEY。"
    ),
    args_model=WebCrawlArgs,
)
def web_crawl(url: str, instructions: str = "", max_pages: int = 10, use_compression: bool = True) -> str:
    """
    爬取站点多个页面，并发 LLM 压缩后返回结构化结果。
    """
    logger.debug(f"爬取站点: {url}, max_pages={max_pages}")
    ...
    if _has_api_key("FIRECRAWL_API_KEY"):
        try:
            ...
            client = Firecrawl(**kwargs)
            logger.info(f"Firecrawl 爬取: {url}, limit={max_pages}")
            ...
            for item in data_list:
                if isinstance(item, dict):
                    ...
                    pages.append({
                        "url": getattr(item, "url", meta.get("sourceURL", "")),
                        "title": meta.get("title", ""),
                        "content": item.markdown or "",
                    })

            logger.info(f"Firecrawl 爬取完成: {len(pages)} 页")
        except Exception as e:
            logger.warning(f"Firecrawl 爬取失败: {e}，尝试 Tavily")

    # ── Tavily（降级）────────────────────────────────────────────────────────
    if not pages and _has_api_key("TAVILY_API_KEY"):
        try:
            payload: dict = {"url": url, "limit": max_pages}
            if instructions:
                payload["instructions"] = instructions
            resp = requests.post(
                "https://api.tavily.com/crawl",
                json={**payload, "api_key": os.getenv("TAVILY_API_KEY")},
                timeout=120,
            )
            if resp.status_code == 200:
                data = resp.json()
                ...
                logger.info(f"Tavily 爬取完成: {len(pages)} 页")
            else:
                logger.warning(f"Tavily crawl 返回 {resp.status_code}")
        except Exception as e:
            logger.warning(f"Tavily 爬取失败: {e}")

    if not pages:
        return (
            "错误：爬取失败，所有后端均不可用。\n"
            "需要 FIRECRAWL_API_KEY 或 TAVILY_API_KEY。\n"
            "也可改用 web_extract 逐页抓取。"
        )
    ...
    if use_compression:
        ...
        def _compress_page(page: dict) -> dict:
            content = page.get("content", "")
            if not content:
                return page
            content = _clean_base64_images(content)
            processed = _process_content_with_llm(content, page.get("url", ""), page.get("title", ""))
            if processed:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(pages), 4)) as executor:
            pages = list(executor.map(_compress_page, pages))
    else:
        for page in pages:
            page["content"] = _clean_base64_images(page.get("content", ""))

    # ── 格式化输出 ────────────────────────────────────────────────────────────
    lines = [f"# 爬取结果：{url}（共 {len(pages)} 页）\n"]
    for i, page in enumerate(pages, 1):
        title = page.get("title") or page.get("url", f"第 {i} 页")
